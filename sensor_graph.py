...
def get_sensor_history(sensor_id):
    ...
    if meta_response.status_code != 200:
        raise Exception(f"could not receive metadata: {meta_response.status_code}")
    ...
    if response.status_code != 200:
        raise Exception(f"metadata error: {response.status_code} – {response.text}")
...
class SensorChartWindow(QMainWindow):
    def __init__(self, sensor_id):
        ...
        self.setWindowTitle(f"Chart: {sensor_id}")
    ...
    def update_plot(self):
        ...
        if self.is_binary:
            ...
            self.plot.plot(x, y, pen=pg.mkPen(color='#0077CC', width=2))
            min_val = np.min(y)
            max_val = np.max(y)
        ...
        self.info_label.setText(f"{self.sensor_id} | Last update: {now} | Next update in: {self.seconds_left}s")
